...
return format.toDisplay(date,format,language);function UTCDate(){return new Date(Date.UTC.apply(Date,arguments))}
var ndate=new Date(date);var val={d:ndate.getDate(),D:_.initials.dates[language].daysShort[ndate.getDay()],DD:_.initials.dates[language].days[ndate.getDay()],m:ndate.getMonth()+1,M:_.initials.dates[language].monthsShort[ndate.getMonth()],MM:_.initials.dates[language].months[ndate.getMonth()],yy:ndate.getFullYear().toString().substring(2),yyyy:ndate.getFullYear()};val.dd=(val.d<10?'0':'')+val.d;val.mm=(val.m<10?'0':'')+val.m;date=[];var seps=$.extend([],format.separators);for(var i=0,cnt=format.parts.length;i<=cnt;i++){if(seps.length)
...
EvoCalendar.prototype.resize=function(){var _=this;var windowW=$(window).width();if(windowW<=_.$breakpoints.tablet){var hasSidebar=!_.$elements.calendarEl.hasClass('sidebar-hide');var hasEvent=!_.$elements.calendarEl.hasClass('event-hide');if(hasSidebar)_.toggleSidebar();if(hasEvent)_.toggleEventList();$(window).off('click.evocalendar.evo-'+_.instanceUid).on('click.evocalendar.evo-'+_.instanceUid,$.proxy(_.toggleOutside,_))}else{$(window).off('click.evocalendar.evo-'+_.instanceUid)}}
...
EvoCalendar.prototype.selectYear=function(event){var _=this;var el,yearVal;if(typeof event==='string'||typeof event==='number'){if((parseInt(event)).toString().length===4){yearVal=parseInt(event)}}else{el=$(event.target).closest('[data-year-val]');yearVal=$(el).data('yearVal')}
if(yearVal=="prev"){--_.$active.year}else if(yearVal=="next"){++_.$active.year}else if(typeof yearVal==='number'){_.$active.year=yearVal}
_.buildSidebarYear();_.buildCalendar()};EvoCalendar.prototype.selectMonth=function(event){var _=this;if(typeof event==='string'||typeof event==='number'){if(event>=0&&event<=_.$label.months.length){_.$active.month=(event).toString()}}else{_.$active.month=$(event.currentTarget).data('monthVal')}
